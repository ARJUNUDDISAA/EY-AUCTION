...
export default function AuctionManagement() {
  ...
  const handleAddAuction = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8070/auction/add",
        newAuction
      );
  ...
  const handleUpdateAuction = async () => {
    if (
      editingAuction &&
      !validateFutureDateTime(editingAuction.startingDateTime)
    ) {
      alert("Starting date and time must be in the future.");
      return;
    }
    ...
    try {
      const response = await axios.put(
        `http://localhost:8070/auction/update/${editingAuction._id}`,
        editingAuction
      );
      alert(response.data.message);
      fetchAuctions();
  ...
  const handleDeleteAuction = async (id) => {
    try {
      const response = await axios.delete(
        `http://localhost:8070/auction/delete/${id}`
      );
      alert(response.data.message);
      fetchAuctions();
    } catch (error) {
      ...
      <div className="mb-4">
        ...
        <div className="mb-3">
          <select
            ...
            onChange={(e) =>
              setNewAuction({ ...newAuction, category: e.target.value })
            }
        ...
        <div className="mb-3">
          ...
          >
            ...
            <option value="BMICH Colombo">ANDHRA PRADESH</option>
            <option value="KCC Kandy">HYDERABAD</option>
            <option value="Negambo">BANGLORE</option>
          </select>
        </div>
        <textarea
        ...
        />
        <div className="mb-3">
          <label htmlFor="startingDateTime" className="form-label">
          ...
          <input
            ...
            onChange={(e) =>
              setNewAuction({ ...newAuction, startingDateTime: e.target.value })
            }
            className="form-control mb-2"
            min={currentDateTime} // Set minimum to current date and time
          />
        </div>
        <button onClick={handleAddAuction} className="btn btn-dark">
          Add Auction
        </button>
      </div>
      ...
      {editingAuction && (
        <div className="mb-4">
          ...
          <input
            ...
            onChange={(e) =>
              setEditingAuction({ ...editingAuction, title: e.target.value })
            }
            className="form-control mb-2"
          />
          <div className="mb-3">
          ...
          <input
            ...
            onChange={(e) =>
              setEditingAuction({ ...editingAuction, image: e.target.value })
            }
            className="form-control mb-2"
          />
          <div className="mb-3">
            ...
            <input
              ...
              onChange={(e) =>
                setEditingAuction({
                  ...editingAuction,
                  startingDateTime: e.target.value,
                })
              }
              className="form-control mb-2"
              min={currentDateTime} // Set minimum to current date and time
          ...
          <button onClick={handleUpdateAuction} className="btn btn-dark">
            Update Auction
          </button>
        </div>
      )}
