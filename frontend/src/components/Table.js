    ...
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif', position: 'relative' }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <div style={{ marginLeft: '380px' ,marginTop: '50px', bottom: '0', right: '0', padding: '20px', display: 'flex', flexDirection: 'row', gap: '10px' }}>
          <button onClick={() => handleNavigate('BMICH')} style={navButtonStyle}>Location: ANDHRA</button>
          <button onClick={() => handleNavigate('NegomboRegal')} style={navButtonStyle}>Location: HYDERABAD</button>
          <button onClick={() => handleNavigate('JoashPlaceMaharagama')} style={navButtonStyle}>Location: BANGLORE</button>
        </div>
      </div>
    </div>
  );
